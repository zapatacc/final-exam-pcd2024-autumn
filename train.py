...
@task(name="Hyperparameter tuning rf")
def hyper_parameter_tuning_rf(X_train, X_test, y_train, y_test):
...
def hyper_parameter_tuning_lr(X_train, X_test, y_train, y_test):
    mlflow.sklearn.autolog()
...
def register_models() -> None:
    """
    Registra el modelo "Champion" y "Challenger" para ambos modelos: Random Forest y Logistic Regression.
    """
    MLFLOW_TRACKING_URI = mlflow.get_tracking_uri()
    client = MlflowClient(tracking_uri=MLFLOW_TRACKING_URI)

    # Buscar las mejores ejecuciones para cada métrica (Accuracy para Random Forest y Logistic Regression)
    ...
    if not rf_runs.empty:
        ...
        rf_result = mlflow.register_model(model_uri=rf_run_uri, name="juan-ley-random-forest-prefect")
        client.set_registered_model_alias(name="juan-ley-random-forest-prefect", alias="champion", version=rf_result.version)
    ...
    if len(rf_runs) > 1:
        ...
        challenger_rf_result = mlflow.register_model(model_uri=challenger_rf_run_uri, name="juan-ley-random-forest-prefect")
        client.set_registered_model_alias(name="juan-ley-random-forest-prefect", alias="challenger", version=challenger_rf_result.version)
    ...
    if not lr_runs.empty:
        ...
        lr_result = mlflow.register_model(model_uri=lr_run_uri, name="juan-ley-logistic-regression-prefect")
        client.set_registered_model_alias(name="juan-ley-logistic-regression-prefect", alias="champion", version=lr_result.version)
    ...
    if len(lr_runs) > 1:
        ...
        challenger_lr_result = mlflow.register_model(model_uri=challenger_lr_run_uri, name="juan-ley-logistic-regression-prefect")
        client.set_registered_model_alias(name="juan-ley-logistic-regression-prefect", alias="challenger", version=challenger_lr_result.version)
...
def main_flow(model_type: str = "random_forest") -> None:
    ...
    mlflow.set_experiment(experiment_name=f"juan-ley-{model_type}-model-experiment-prefect")

    # Paso 1: Lectura de datos
    CSV_PATH = "../raw_data/tickets_classification_eng.json"
    ...
    if model_type == "random_forest":
        best_params_rf = hyper_parameter_tuning_rf(X_train, X_test, y_train, y_test)
        best_params_lr = None
    elif model_type == "logistic_regression":
        best_params_lr = hyper_parameter_tuning_lr(X_train, X_test, y_train, y_test)
        best_params_rf = None
    else:
        raise ValueError("El tipo de modelo especificado no es soportado. Use 'random_forest' o 'logistic_regression'.")
